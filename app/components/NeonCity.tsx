...
export default function NeonCity({ scrollProgress }: NeonCityProps) {
  ...
  const stage1 = clamp(scrollProgress / 0.35, 0, 1)   // 0–35%: zoom in
  const stage2 = clamp((scrollProgress - 0.35) / 0.4, 0, 1) // 35–75%: fly through
  const stage3 = clamp((scrollProgress - 0.75) / 0.25, 0, 1) // 75–100%: blur/fade

  // City scale: 1 → 1.6 in stage1, then 1.6 → 2.8 in stage2
  const cityScale = lerp(lerp(1, 1.6, stage1), 2.8, stage2)

  // Buildings split apart in stage2
  ...
  return (
    ...
    >
      ...
      {/* Main city group — scales from center */}
